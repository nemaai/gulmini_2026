...
def load_npy_local(path):
    ...
    if eeg.ndim != 2:
        raise ValueError(f"Expected 2D array, got {eeg.shape}")

    if eeg.shape[0] != 19 and eeg.shape[1] == 19:
        eeg = eeg.T

    elif eeg.shape[0] != 19:
        raise ValueError(f"Cannot find 19 channels in {eeg.shape}")
...
def process_npy_api(
    npy_path: str,
    output_dir: str,
    known_sfreq: float = 256,
):
    """
    Local API version of the pipeline.
    Input  : local .npy
    ...
    eeg = resample_to_contract(eeg, known_sfreq)
    ...
    processed_path = os.path.join(output_dir, "processed.npy")

    np.save(processed_path, eeg)

    # -------------------------
    # SAVE QC
    # -------------------------

    qc_path = os.path.join(output_dir, "qc.json")
